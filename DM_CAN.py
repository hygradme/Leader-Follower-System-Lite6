...
class Motor:
    ...
    def recv_data_temperature(self, t_mos: int, t_rotor: int):
        self.t_mos = t_mos
...
class MotorControl:
    ...
    def control_pos_force(self,Motor,Pos_des:float,Vel_des,i_des):
        ...
        self.send_data_frame[21:29] = data_buf
        self.serial_.write(bytes(self.send_data_frame.T))
        motor_data = self.recv()  # receive the data from serial port
        return motor_data
    ...
    def recv(self):
        ...
        for packet in packets:
            if len(packet) == 16:
                # print("packet", [a for a in packet])

                data = packet[7:15]
                CANID = (packet[6] << 24) | (packet[5] << 16) | (packet[4] << 8) | packet[3]
                CMD = packet[1]
                motor_data = self.process_packet(data, CANID, CMD)
                if len(motor_data) != 0:
                    return motor_data
        return {}
    ...
    def process_packet(self, data, CANID, CMD):
        # print("CANID", CANID, "CMD", CMD,  "data", [val for val in data])
        
        if CMD == 0x11:
            # print(CMD,"CMD is 0x11", CANID)

            SLAVEID = data[0] & 0x0F  # Lower 8 bits for ID
            ERR = (data[0] >> 4) & 0x0F  # Upper 4 bits for ERR

            # print(f"Extracted ID: {SLAVEID}, ERR: {ERR}")

            # debug purpose since CANID is received as 0

            if SLAVEID in self.motors_map:
                # print(SLAVEID, "SLAVEID is in motormap")
                q_uint = np.uint16((np.uint16(data[1]) << 8) | data[2])
                dq_uint = np.uint16((np.uint16(data[3]) << 4) | (data[4] >> 4))
                tau_uint = np.uint16(((data[4] & 0xf) << 8) | data[5])
                MotorType_recv = self.motors_map[SLAVEID].MotorType
                ...
                self.motors_map[SLAVEID].recv_data(recv_q, recv_dq, recv_tau)

                # Extract temperature information
                T_MOS = data[6]  # MOS temperature in degrees Celsius
                T_Rotor = data[7]  # Rotor temperature in degrees Celsius
                self.motors_map[SLAVEID].recv_data_temperature(T_MOS, T_Rotor)

                # Print extracted information for debugging
                # print(f"ID: {SLAVEID}, ERR: {ERR}, pos:{recv_q}, speed:{recv_dq}, torque:{recv_tau}, T_MOS: {T_MOS}°C, T_Rotor: {T_Rotor}°C")
                return {
                    "ID": SLAVEID,
                    "ERR": ERR,
                    "pos": recv_q,
    ...
    def addMotor(self, Motor):
        ...
        :param Motor: Motor object 电机对象
        """
        # print("slaveID", Motor.SlaveID, "masterID", Motor.MasterID)
    ...
    def get_motor_data(self, Motor):
        # cmd = np.uint8(0xFC)
        data_buf = np.array([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, np.uint8(0xFC)], np.uint8)
        self.send_data_frame[13] = np.uint8(Motor.SlaveID)
        self.send_data_frame[21:29] = data_buf
        self.serial_.write(bytes(self.send_data_frame.T))
        # print(f"write {Motor.SlaveID} to get data")
        precise_delay(0.5)

        # self.recv()  # receive the data from serial port
        receive_target_slaveID = False
        retry_count = 0
        while not receive_target_slaveID and retry_count < 1:
            data_recv = self.serial_.read_all()
            packets = extract_packets(data_recv)
            for packet in packets:
                ...
                if CMD == 0x11:
                    ...
                    # if SLAVEID in self.motors_map:
                    # print("SLAVEID", SLAVEID, Motor.SlaveID)
                    if SLAVEID != Motor.SlaveID:
                        retry_count += 1
                    else:
                        # print(SLAVEID, "SLAVEID is in motormap")
                        q_uint = np.uint16((np.uint16(data[1]) << 8) | data[2])
